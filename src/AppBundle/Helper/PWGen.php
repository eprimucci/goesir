...
class PWGen {
     ...
     */
    public function __construct($length = 8, $secure = false, $numerals = true, $capitalize = true, $ambiguous = false, $no_vovels = false, $symbols = false) {
        self::__static();
            ...
            while ($c < $this->pw_length) {
                ...
                if ($this->pwgen_flags & self::PW_SYMBOLS) {
                    if (!$first && (self::my_rand(0, 9) < 2)) {
                        do {
                            $ch = self::$pw_symbols[self::my_rand(0, strlen(self::$pw_symbols) - 1)];
                        } while (($this->pwgen_flags & self::PW_AMBIGUOUS) &&
                        strpos(self::$pw_ambiguous, $ch) !== false);
                        $this->password[$c++] = $ch;
